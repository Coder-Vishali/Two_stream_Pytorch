...
def make_dataset(root, source):
    ...
    if not os.path.exists(source):
        print("Setting file %s for gms dataset doesn't exist." % (source))
        sys.exit()
    else:
        clips = []
        with open(source) as split_f:
            ...
            for line in data:
                ...
                # print("line_info", line_info[0].rsplit("_", 1)[0])
                clip_path = os.path.join(root,  line_info[0].rsplit("_", 1)[0], line_info[0])
...
def ReadSegmentRGB(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    for offset_id in range(len(offsets)):
        ...
        for length_id in range(1, new_length + 1):
            frame_name = name_pattern % (length_id + offset - 1)
...
def ReadSegmentFlow(path, offsets, new_height, new_width, new_length, is_color, name_pattern):
    ...
    # print("outside new_length", new_length)
    # print("outside offset", offsets)
    ...
    for offset_id in range(len(offsets)):
        ...
        if offset!=0:
            offset = offset - 1
            for length_id in range(1, new_length + 1):
                frame_name_x = name_pattern % ("x", length_id + offset - 1)
                frame_path_x = path + "/" + frame_name_x
                # print("frame_path_x:", frame_path_x)
    ...
    def __init__(self,
        ...
        if name_pattern:
            ...
            if self.modality == "rgb":
                self.name_pattern = "img_%d.jpg"
            elif self.modality == "flow":
                self.name_pattern = "flow_%s_%d.jpg"
